...
namespace CRUD.Controllers
{
    ...
    public class ExamScoreController : ControllerBase
    {
        ...
        public ExamScoreController(CRUDContext dbContext)
        {
            _dbContext = dbContext;
        }

        [HttpGet]
        public async Task<ActionResult<IEnumerable<ExamScore>>> GetBrands()
        {
            var examscores = await _dbContext.ExamScores
        .Include(e => e.Student) // Lấy thông tin Student cho mỗi ExamScore
        .Include(e => e.Course)  // Lấy thông tin Course cho mỗi ExamScore
        .ToListAsync();
